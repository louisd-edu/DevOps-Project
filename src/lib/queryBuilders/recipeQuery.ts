 ...
 */
export function buildRecipeQuery(supabase: SupabaseClient, options: RecipeQueryOptions) {
	const {
		cuisines = [],
		searchText = '',
		sortBy = 'name',
	...
	if (sortBy === 'name') {
		...
		query = query.order('cookingtime', { ascending: sortDir === 'asc', nullsFirst: false });
	}
