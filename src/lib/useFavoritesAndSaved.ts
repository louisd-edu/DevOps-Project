...
export function useFavoritesAndSaved(sb: SupabaseClient) {
  ...
  const favUnsub = favoriteIdsStore.subscribe((v) => {
    favSnapshot = v;
  });
  const savedUnsub = savedIdsStore.subscribe((v) => {
    savedSnapshot = v;
  });
  ...
  async function loadFavorites() {
    ...
    favoriteIdsStore.set(
      new Set(
        (favs ?? []).map((r: { recipeid: string | number }) =>
          String(r.recipeid),
        ),
      ),
    );
  ...
  async function loadSaved() {
    ...
    savedIdsStore.set(
      new Set(
        (rows ?? []).map((r: { recipeid: string | number }) =>
          String(r.recipeid),
        ),
      ),
    );
  ...
  async function toggleFavorite(recipeId: string | number) {
    ...
    if (isFav) {
      ...
      if (error) {
        ...
        if (typeof window !== "undefined")
          alert(`Could not add favorite: ${error.message}`);
        return;
      }
      favoriteIdsStore.update((s) => {
        const n = new Set(s);
  ...
  async function toggleSaved(recipeId: string | number) {
    ...
    if (isSaved) {
      ...
      if (error) {
        ...
        if (typeof window !== "undefined")
          alert(`Could not remove saved: ${error.message}`);
        return;
      }
      savedIdsStore.update((s) => {
        const n = new Set(s);
        n.delete(key);
        return n;
      });
