<script lang="ts">
    ...
    let { data } = $props<{ data: { savedrecipes: Recipe[], isOwner: boolean, isPrivate: boolean } }>();
...
{#if data.isPrivate && !data.isOwner}
	<div class="privacy-message">
		...
		<p>The owner has chosen to keep their saved recipes private.</p>
	</div>
{:else if data.savedrecipes.length === 0}
